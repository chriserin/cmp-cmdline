...
local definitions = {
	{
		...
		exec = function(option, arglead, cmdline, force)
			...
			for _, item in ipairs(items) do
				if not is_magic_file and not string.find(item.word, fixed_input, 1, true) then
				...
				if is_magic_file then
					...
					item.label = fixed_input .. item.word
					item.word = nil
				end
			end
			return items
...
source.complete = function(self, params, callback)
	...
	items = vim.tbl_map(function(item)
		if type(item) == "string" then
			item = { label = item, filterText = nil, kind = kind }
		elseif item.label then
			item.kind = kind
		else
			item = { label = item.word, filterText = item.filterText, kind = kind }
		end
